...
import { updateDataStatistikAction, deleteDataStatistikAction } from "../../actions";
...
export default function UbahDataStatistikPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<DataStatistikInput>({
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (error || !data) {
        ...
        reset({
          ...data,
          jumlah_penduduk: data.jumlah_penduduk || undefined,
        });
      }
      setFetching(false);
    }
  ...
  async function onSubmit(data: DataStatistikInput) {
    ...
    const res = await updateDataStatistikAction(id, formData);
    if (res?.error) {
      setError(res.error);
      setLoading(false);
    }
  ...
  async function handleDelete() {
    if (!confirm("Yakin ingin menghapus data statistik ini? Aksi ini tidak dapat dibatalkan.")) return;
    setLoading(true);
    const res = await deleteDataStatistikAction(id);
    if (res?.error) {
      setError(res.error);
      setLoading(false);
    } else {
  ...
  return (
    <div className="max-w-xl space-y-6">
      <div>
        ...
        <h1 className="font-display text-2xl font-semibold text-primary">Ubah Data Statistik</h1>
      </div>
